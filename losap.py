...
def swap_name_order(df_def):
    ...
    for i in range(len(df_def)):
        fname = df_def.loc[i, "Member Name"].lstrip()
        if (fname.find(',') > 0):
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.losap_rows_to_skip = 10     # Skip this number of rows before reading data
    ...
    def import_epcr(self):
        ...
        if file_name:
            try:
                ...
                # Reverse first and last names 
                new = df_ePCR['Member Name'].str.split(" ", n = 1, expand = True) 
        ...
        if directory:
            try:
                ...
                if df_losap.shape[0] > 0:
                    ...
                    df_losap_drills = df_losap_drills.groupby(['Member Name'])['Drills'].agg('sum').reset_index()
        ...
        if file_name:
            try:
                with pd.ExcelWriter(file_name, engine='xlsxwriter') as writer:  
                    self.df.to_excel(writer, sheet_name = self.output_worksheet_name)
